      ...
      <div class="bg-light p-5 rounded">
        ...
        <form action="{{route('products.update',$product->id)}}" method="POST" class="px-md-5"enctype="multipart/form-data">
          ...
          <div class="form-group mb-3 row">
            ...
            <div class="col-md-10">
              <textarea name="short_description" id="" cols="30" rows="5" class="form-control py-2">{{old('short_description',$product->short_description)}}</textarea>
          ...
          <div class="col-sm-10">
          <img src="{{asset('assests/images/product/'.$product->image)}}" alt="{{ $product->carTitle }}" width="100">
          <input type="file" class="form-control" id="image" name="image" value="{{old('image',$product->image)}}">{{--{{old('name,$default')}} --}}
          </div>
         </div>
